...
export default function ChooseMaterial({ mode = 'viva' }) {
  const navigate = useNavigate();
  const fileInputRef = useRef(null);
  const isRecall = mode === 'recall';
  const sessionLabel = isRecall ? 'recall' : 'viva';
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const response = await fetch(`http://localhost:3000/${isRecall ? 'recall' : 'material_analysis'}`, {
        method: 'POST',
        headers: {
      ...
      if (response.ok && data.success) {
        ...
        setStatusMessage(data.message || (isRecall ? 'Material analyzed for active recall.' : 'Material analyzed and 10 viva questions generated by Gemini AI!'));
        setConfirmedPath(data.filePath || selectedMaterial.absolutePath);
        setAiAnalysisResult(data);

        sessionStorage.setItem(isRecall ? 'active_recall_session' : 'active_viva_session', JSON.stringify({
      ...
      setErrorMessage(`Gemini ${sessionLabel} analysis request failed: ${err.message}. Please verify the backend is running and API_KEY is valid in backend/.env.`);
    } finally {
  ...
  const handleStartSession = () => {
    navigate(isRecall ? '/recall-session' : '/voice-session');
  };
  ...
  return (
    <div className="space-y-6 md:space-y-8 pb-12 max-w-5xl mx-auto">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 bg-[#393E46]/40 p-6 md:p-8 rounded-3xl border border-[#948979]/20 backdrop-blur-sm">
        <div>
          <Link
            to={isRecall ? '/active-recall' : '/viva-sessions'}
          ...
          >
            ...
            <span>Back to {isRecall ? 'Active Recall Sessions' : 'AI Viva Sessions'}</span>
          </Link>
          <h1 className="text-2xl md:text-4xl font-extrabold text-[#DFD0B8] tracking-tight font-['Outfit']">
            Choose Material for {isRecall ? 'Recall' : 'Viva'}
          </h1>
          <p className="text-xs md:text-sm text-[#DFD0B8]/70 mt-1">
            Select course material from your assets or upload a new file to prepare your {isRecall ? 'active recall' : 'viva examination'} session
          </p>
        </div>
      ...
      <form onSubmit={handleSubmit} className="space-y-6">
        ...
        <div className="bg-[#393E46] rounded-3xl p-6 md:p-8 border border-[#948979]/30 shadow-xl space-y-5">
          <div className="pb-3 border-b border-[#948979]/20">
            <h2 className="text-lg md:text-xl font-bold text-[#DFD0B8] flex items-center gap-2">
              ...
              <span>2. Choose {isRecall ? 'Recall' : 'Viva'} Duration</span>
            </h2>
            <p className="text-xs text-[#948979] mt-0.5">
              Select a time limit or choose no limit for this {isRecall ? 'active recall' : 'technical examination'} session
            </p>
          </div>
        ...
        <div className="bg-[#393E46] rounded-3xl p-6 md:p-8 border border-[#948979]/30 shadow-xl space-y-4">
          <div className="flex flex-col sm:flex-row items-center justify-between gap-4">
            <div>
              <p className="text-sm font-bold text-[#DFD0B8]">Ready to Analyze Material for {isRecall ? 'Recall' : 'Viva'}</p>
              <p className="text-xs text-[#948979]">
                Target endpoint: <code className="text-[#DFD0B8] bg-[#222831] px-1.5 py-0.5 rounded">localhost:3000/{isRecall ? 'recall' : 'material_analysis'}</code>
              </p>
            </div>
            ...
            >
              ...
              <span>{isSubmitting ? 'Analyzing Material...' : `Submit ${isRecall ? 'Recall' : 'Material'} Analysis`}</span>
            </button>
          </div>
          ...
          {submitSuccess && (
            <div className="p-6 md:p-8 rounded-3xl bg-[#222831] border-2 border-emerald-500/40 space-y-6 animate-in fade-in zoom-in-95">
              <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 pb-4 border-b border-[#948979]/20">
                ...
                >
                      <span>Start {isRecall ? 'Recall' : 'Session'}</span>
              ...
              {aiAnalysisResult && (
                <div className="space-y-4">
                  ...
                  <div className="pt-4 border-t border-[#948979]/20 flex justify-end">
                    ...
                    >
                      <span>{isRecall ? 'Start Recall Session' : 'Start Live Viva Session'}</span>
                      <ArrowRight className="w-4 h-4 text-[#222831]" />
                    </button>
                  </div>
                </div>
